...
struct get_pointer<Alloc, tinystl::void_t<typename Alloc::pointer>> {
...
struct get_const_pointer {
  using pointer_type = typename get_pointer<Alloc>::type;
...
struct get_rebind_alloc {
  using type = typename get_rebind_get_impl<Alloc, NewAlloc>::type;
};
...
template <class Alloc, class = void>
const bool has_select_on_container_copy_construction = false;
...
struct allocator_traits {
  ...
  using const_void_pointer =
    typename detail::get_const_void_pointer<Alloc>::type;
  ...
  using propagate_on_container_copy_assignment =
    typename detail::get_propagate_on_container_copy_assignment<Alloc>::type;
  using propagate_on_container_move_assignment =
    typename detail::get_propagate_on_container_move_assignment<Alloc>::type;
  using propagate_on_container_swap =
    typename detail::get_propagate_on_container_swap<Alloc>::type;

  using is_always_equal = typename detail::get_is_always_equal<Alloc>::type;

  // Member alias templates
  template <class NewAlloc>
  using rebind_alloc = typename detail::get_rebind_alloc<Alloc, NewAlloc>::type;
  ...
  template <
    class U = Alloc,
    enable_if_t<detail::has_select_on_container_copy_construction<U>, int> = 0>
